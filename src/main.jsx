...
function App(){
 const [data,setData]=useState(null); const [current,setCurrent]=useState(0); const [turn,setTurn]=useState(null); const lock=useRef(false); const goRef=useRef(null);
 useEffect(()=>fetch('/public_data.json').then(r=>r.json()).then(setData),[]);
 useEffect(()=>{const onKey=e=>{if(e.key==='ArrowRight'){e.preventDefault();goRef.current?.(1)}else if(e.key==='ArrowLeft'){e.preventDefault();goRef.current?.(-1)}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey)},[]);
 if(!data)return <div className="loading">Loading Golden Jubilee…</div>;
 const pages=[];pages.push(<Cover key="cover" title={data.book?.title||'Golden Jubilee 50th Anniversary E-Book'}/>);pages.push(<Journey key="journey"/>);
 (data.participants||[]).forEach((p,i)=>{pages.push(<Profile key={p.participant_id+'p'} p={p} index={i} n={pages.length+1}/>);if(Object.keys(p.photos||{}).some(k=>k!=='recent'))pages.push(<Memory key={p.participant_id+'m'} p={p} n={pages.length+1}/>)});pages.push(<Closing key="closing"/>);
 const go=dir=>{if(lock.current)return;const target=current+dir;if(target<0||target>=pages.length)return;lock.current=true;setTurn({from:current,to:target,dir});setTimeout(()=>{setCurrent(target);setTurn(null);lock.current=false},TURN_MS)};
 goRef.current=go;
 const shown=turn?turn.to:current; const outgoing=turn?turn.from:null;
 return <div className="app">
  <header><div><div className="kicker">GOLDEN JUBILEE • 50 YEARS</div><h1>{data.book?.title||'Golden Jubilee'}</h1></div><nav><button onClick={()=>go(-1)} disabled={current===0||!!turn}>‹</button><span>{current+1} / {pages.length}</span><button onClick={()=>go(1)} disabled={current===pages.length-1||!!turn}>›</button></nav></header>
  <main className="stage"><div className="book">
   <div className="page-layer base">{pages[shown]}</div>
   {turn&&<div className={`page-layer turn ${turn.dir>0?'next':'prev'}`}>{pages[outgoing]}</div>}
  </div></main>
  <footer><span>Use the buttons or keyboard ← → to turn pages</span><span>{current===0?'Cover':current===pages.length-1?'Closing':'Golden Jubilee'}</span></footer>
 </div>
}
